...
@Component
public class CCCodeGenerationAdapter implements CCodeGenerationPort {

    public static final String C_FOLDER_NAME = "CCodeFiles/";

    public static final String MAIN_C_FILE = C_FOLDER_NAME + "main.c";
    public static final String REGEX_ACCEPTOR_H_FILE = C_FOLDER_NAME + "regexAcceptor.h";
    public static final String REGEX_ACCEPTOR_C_FILE = C_FOLDER_NAME + "regexAcceptor.c";
    public static final String MAKEFILE_FILE = C_FOLDER_NAME + "Makefile";

    @Override
    public HashMap<String, String> generateFromMinDFA(MinDFAEntity minDFA) {
        HashMap<String, String> generatedFiles = new HashMap<>();

        generatedFiles.put(MAIN_C_FILE, generateMainFileContent());
        generatedFiles.put(REGEX_ACCEPTOR_H_FILE, generateRegexAcceptorHFileContent(minDFA));
        generatedFiles.put(REGEX_ACCEPTOR_C_FILE, generateRegexAcceptorFileContent(minDFA));
        generatedFiles.put(MAKEFILE_FILE, generateMakefileFileContent());
    ...
    private String generateRegexAcceptorHFileContent(MinDFAEntity minDFA) {
        ...
        for (int state = 0; state < minDFA.stateCount(); state++) {
            ...
            sb.append("void ").append(stateName).append("(char currentChar);\n");
        }
