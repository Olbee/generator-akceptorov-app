...
// TODO: rework
@Component
public class JavaCodeFilesGenerationAdapter extends AbstractCodeFilesGenerationAdapter
    implements JavaCodeGenerationPort {
  ...
  private String generateMainFileContent() {
    ...
    sb.append(
        "\t\t\t\t\tSystem.out.println(\"Retazec je prilis dlhy (maximalna dlzka je 64).\");\n");
    sb.append("\t\t\t\t\tcontinue;\n");
    sb.append("\t\t\t\t}\n");
  ...
  private String generateRegexAcceptorFileContent(MinDFAEntity minDFA) {
    ...
    final int[] acceptedStates =
        minDFA.acceptStates().stream().mapToInt(Integer::intValue).toArray();
    ...
    if (acceptedStates.length == 0) {
      ...
      sb.append("\t\treturn (currentState == ")
          .append(acceptedStates[0])
          .append(") ? ACCEPT : NON_ACCEPT;\n");
    } else {
      sb.append("\t\treturn (currentState == ").append(acceptedStates[0]).append("\n");
      for (int state = 1; state < acceptedStates.length; state++) {
        String lastState = state + 1 == acceptedStates.length ? ") ? ACCEPT : NON_ACCEPT;" : "";
        sb.append("\t\t\t|| currentState == ")
            .append(acceptedStates[state])
            .append(lastState)
            .append("\n");
      }
    }
