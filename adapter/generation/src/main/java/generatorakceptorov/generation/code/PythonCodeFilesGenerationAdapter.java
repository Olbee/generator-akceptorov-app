...
// TODO: rework
@Component
public class PythonCodeFilesGenerationAdapter extends AbstractCodeFilesGenerationAdapter
    implements PythonCodeGenerationPort {
  ...
  private String generateRegexAcceptorFileContent(MinDFAEntity minDFA) {
    ...
    if (acceptedStates.length == 0) {
      ...
      sb.append("\t\treturn self.ACCEPT if self.current_state == ")
          .append(acceptedStates[0])
          .append(" else self.NON_ACCEPT\n");
    } else {
      ...
      for (int stateCounter = 0; stateCounter < acceptedStates.length; stateCounter++) {
        ...
        if (stateCounter + 1 != acceptedStates.length) sb.append(", ");
      }
      sb.append("):\n");
      sb.append("\t\t\treturn self.ACCEPT\n");
      sb.append("\t\telse:\n");
