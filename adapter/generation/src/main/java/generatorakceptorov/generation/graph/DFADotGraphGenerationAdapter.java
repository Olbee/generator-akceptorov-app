...
public class DFADotGraphGenerationAdapter extends AbstractDotGraphGenerationAdapter implements DFADotGraphGenerationPort {
    ...
    public String generateFromMinDFA(MinDFAEntity entity) {
        ...
        for (int i = 0; i < entity.transitions().length; i++) {
            String shape =
                entity.acceptStates().contains(i)
                    ? "doublecircle"
                    : "ellipse";
            sb.append("    ").append(i).append(" [label=\"q").append(i).append("\", shape=").append(shape).append("];\n");
        }
        ...
        for (String key : uniqueTransitions.keySet()) {
            ...
            sb.append("    ").append(sourceState).append(" -> ").append(targetState).append(" [dir=forward, label=<").append(symbols).append(">];\n");
        }
        sb.append("}");
