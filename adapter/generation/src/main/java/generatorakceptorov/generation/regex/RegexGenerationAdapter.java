...
public class RegexGenerationAdapter implements RegexGenerationPort {
    ...
    if (command.notationType() == COMMON) {
      if (!command.regexNumOfSymbols().equals("") || !command.regexSymbolsToUse().equals("")) {
        while (randomRegexSymbolsCount != (Integer.parseInt(command.regexNumOfSymbols()))) {
          ...
          randomRegex = addAlternations(addQuantifiers(generateRandomExpression(symbolsCount)));
        ...
        return addAlternations(addQuantifiers(generateRandomExpression(symbolsCount)));
      }
    } else if (command.notationType() == SIMPLE) {
      if (!command.regexNumOfSymbols().equals("") || !command.regexSymbolsToUse().equals("")) {
        while (randomRegexSymbolsCount != (Integer.parseInt(command.regexNumOfSymbols()))) {
          randomRegexSymbolsCount = 0;
          randomRegex = addAlternations(generateRandomExpression(symbolsCount));
          for (int i = 0; i < randomRegex.length(); i++)
            if (command.regexSymbolsToUse().contains(String.valueOf(randomRegex.charAt(i))))
              randomRegexSymbolsCount++;
        }
      } else {
        return addAlternations(generateRandomExpression(symbolsCount));
      }
    } else {
      throw UNEXPECTED_REGEX_NOTATION_TYPE_ERROR
          .createError(command.notationType().toString())
  ...
  private void initialize(RandomRegexCommand command) {
    ...
    if (command.notationType() == COMMON) {
      // * by 3 -> just to generate approximately the same length common regex as the simple
      // notation regex.
      // By default, the simple notation regex will be bigger due to all the possible grouping
      // symbols in it,
    ...
    if (!command.regexNumOfSymbols().equals("")) {
      ...
      int randomAlphabetCount =
          ThreadLocalRandom.current().nextInt(OPERANDS_COUNT, newMaxOperandsCount + 1);
  ...
  public String generateRandomExpression(int symbolsCount) {
        ...
        case 2 -> {
          if (symbolCount < groupingSymbols.length) {
            ...
            if (index == 0 || index == result.length()) {
              ...
              if (!isGroupingSymbol(result.charAt(index - 1))
                  && !isGroupingSymbol(result.charAt(index))) {
                result.insert(index, groupingSymbol.charAt(0));
                result.insert(index + 1, groupingSymbol.charAt(1));
                symbolCount++;
              }
    ...
    if (result.length() == symbolsCount) return result.toString();
  ...
  private StringBuilder ensureNonEmptyGroupingSymbols(StringBuilder sb, String symbol) {
    ...
    while (openingIndex >= 0) {
      ...
      if (closingIndex == openingIndex + 1) {
        sb.insert(openingIndex + 1, generateRandomExpression(random.nextInt(symbolsCount) + 1));
  ...
  private boolean isGroupingSymbol(char symbol) {
    for (String groupingSymbol : groupingSymbols)
      if (groupingSymbol.contains(String.valueOf(symbol))) return true;
