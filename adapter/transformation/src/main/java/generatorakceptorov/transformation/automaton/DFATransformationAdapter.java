...
// TODO: rework
@Component
public class DFATransformationAdapter extends AbstractTransformationAdapter
    implements DFATransformationPort {
          ...
          if (!statesFirst.isEmpty() && !statesSecond.isEmpty()) {
            ...
            if (currentPartition.contains(transition)) {
              ...
              currentPartition.add(
                  statesFirst.size() <= statesSecond.size() ? statesFirst : statesSecond);
            }
          } else {
  ...
  private TreeSet<Integer> resolveNotAcceptDfaStates(DFAEntity entity) {
    ...
    for (int state = 0; state < entity.stateCount(); state++)
      if (!entity.acceptStates().contains(state)) notAcceptDfaStates.add(state);

    return notAcceptDfaStates;
  }

  private Integer[][] resolveTransitions(
      DFAEntity entity,
  ...
  private TreeSet<Integer> resolveAcceptStates(
      DFAEntity entity, ArrayList<TreeSet<Integer>> dfaToMinDfaStateTransitions) {
