...
// TODO: rework
@Component
public class RegexTransformationAdapter extends AbstractTransformationAdapter
    implements RegexTransformationPort {
  ...
  public NFAEntity transformToNFA(RegexEntity entity) {
    ...
    for (char currentChar : entity.regex().toCharArray()) {
      if (currentChar == CONCATENATION) {
        ...
        if (currentChar == OR)
          handleOr(startState, acceptState, transitions, startStates, acceptStates);
        else if (currentChar == STAR)
          handleStar(startState, acceptState, transitions, startStates, acceptStates);
        else if (currentChar == PLUS)
          handlePlus(startState, acceptState, transitions, startStates, acceptStates);
        else if (currentChar == QUESTION)
          handleQuestion(startState, acceptState, transitions, startStates, acceptStates);
        else
          handleOperand(
  ...
  private void handleConcatenation(
      List<Integer>[][] transitions, Stack<Integer> startStates, Stack<Integer> acceptStates) {
  ...
  private ArrayList<Character> resolveAlphabet(RegexEntity entity) {
    ...
    alphabet.add('$');
    for (char currentChar : entity.regex().toCharArray()) {
      if (isOperand(currentChar) && !alphabet.contains(currentChar)) alphabet.add(currentChar);
    }
  ...
  private int resolveStatesCount(RegexEntity entity) {
    ...
    for (char currentChar : entity.regex().toCharArray())
      if (isOperand(currentChar)
          ...
          || currentChar == PLUS) statesCount += 2;

    return statesCount;
  }

  private ArrayList<Integer>[][] resolveTransitions(
      ArrayList<Character> alphabet, int statesCount) {
    final ArrayList<Integer>[][] transitions = new ArrayList[statesCount][alphabet.size()];
    for (int i = 0; i < statesCount; i++)
      for (int j = 0; j < alphabet.size(); j++) transitions[i][j] = new ArrayList<>(2);
