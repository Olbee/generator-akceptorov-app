...
@Component
public class StringTransformationAdapter extends AbstractTransformationAdapter
    implements StringTransformationPort {
  ...
  public RegexEntity transformToRegex(String regex, NotationType notationType) {
    if (notationType == SIMPLE) {
      ...
      if (!commonRegexIsCorrect(regexInCommonNotation)
          || regex.matches(".*[+*?].*")) // || regex contains * or + or ?
      throwInvalidRegexSyntaxErrorError(regex);
      return ImmutableRegexEntity.of(
          toPostfix(addConcatenation(regexInCommonNotation)), notationType);
    } else if (notationType == COMMON) {
      if (!commonRegexIsCorrect(regex)) throwInvalidRegexSyntaxErrorError(regex);
      return ImmutableRegexEntity.of(toPostfix(addConcatenation(regex)), notationType);
    } else {
  ...
  private void throwInvalidRegexSyntaxErrorError(String regex) {
    throw INVALID_REGEX_SYNTAX_ERROR.createError(regex).convertToException();
  }
  ...
  private String addConcatenation(String regex) {
    ...
    for (int i = 1; i < regex.length(); i++) {
      ...
      if ((isOperand(leftSymbol)
              || leftSymbol == STAR
              || leftSymbol == QUESTION
              || leftSymbol == PLUS
              || leftSymbol == RIGHT_PARENTHESIS)
  ...
  private String toPostfix(String regex) {
    ...
    final Stack<Character> stack = new Stack<>();
    for (char currentChar : regex.toCharArray()) {
      if (isOperand(currentChar)) sb.append(currentChar);
      else {
        if (currentChar == LEFT_PARENTHESIS) stack.push(currentChar);
        else if (currentChar == RIGHT_PARENTHESIS) {
          while (!(stack.peek() == LEFT_PARENTHESIS)) sb.append(stack.pop());
          stack.pop();
        } else {
          while (!stack.isEmpty()
              && getOperatorsPriority(stack.peek()) >= getOperatorsPriority(currentChar))
            sb.append(stack.pop());
          stack.push(currentChar);
        }
      }
  ...
  public boolean commonRegexIsCorrect(String regex) {
    final Map<Character, Integer> operatorsDepth =
        new HashMap<>(
            Map.of(
                LEFT_PARENTHESIS, 0,
                CONCATENATION, 0,
    ...
    for (char currentChar : regex.toCharArray()) {
      if (!symbolIsOk(currentChar)) return false;
      switch (currentChar) {
        ...
        case RIGHT_PARENTHESIS -> {
          if (operatorsDepth.get(LEFT_PARENTHESIS) == 0) return false;
          operatorsDepth.put(LEFT_PARENTHESIS, operatorsDepth.get(LEFT_PARENTHESIS) - 1);
          clearOperatorsDepth(operatorsDepth, OR, STAR, PLUS, QUESTION);
        }
  ...
  void clearOperatorsDepth(Map<Character, Integer> operatorsDepth, char... operators) {
    for (char operator : operators) operatorsDepth.put(operator, 0);
  }
