...
public class DownloadCodeFilesAsZipService implements DownloadCodeFilesAsZipUseCase {
  ...
  private final Map<ProgrammingLanguage, CodeGenerationPort> codeGenerationPorts;
  ...
  public CodeFilesZipData execute(LanguagesToDownloadCodeCommand command) {
    ...
    try (ZipOutputStream zos = new ZipOutputStream(baos)) {
      command
          .languagesToDownloadCode()
          .forEach(
              language -> {
                try {
                  addFilesToZip(
                      zos, codeGenerationPorts.get(language).generateFromMinDFA(command.minDFA()));
  ...
  private void addFilesToZip(ZipOutputStream zos, HashMap<String, String> codeFiles)
      throws IOException {
  ...
  private void addFileToZip(ZipOutputStream zos, byte[] content, String fileName)
      throws IOException {
  ...
  private void throwCodeFilesToZipGenerateErrorError(String errorMessage) {
    throw CODE_FILES_TO_ZIP_GENERATE_ERROR.createError(errorMessage).convertToException();
  }
}
